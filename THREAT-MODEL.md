...
| local host-trust rollback or capsule substitution | a compromised device presents an older pin/history set or a self-chosen controller | complete signed portable pin/history bytes, out-of-band expected controller fingerprint, strict chain reconstruction, separately preserved prior capsule/hash, prefix-only comparison, explicit non-anchor/non-restore claims; same-device export alone provides no rollback assurance |
