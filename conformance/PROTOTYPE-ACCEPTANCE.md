...
| CRY-012 | Export a complete portable host-trust capsule, verify it without a Soma home using independently supplied controller DID/key hash, then mutate every object byte/path/kind/length/hash, pin, transition, chain order, count, root, controller, claim, capsule ID, and signature. Compare an identical capsule, a strict descendant, a truncated history, a missing host, and a same-height fork against separately preserved trusted bytes. PASS only if the valid and strict-descendant cases succeed, every mutation/rollback/fork fails with no state installation, the export contains no managed secret or unrelated owner data, and packet capture shows zero egress. Same-device creation MUST remain labeled non-anchor and non-restore authority. | EV-CLI, EV-CRY, EV-STATE, EV-SCAN, EV-NET | P0 |
